...
int lab25task3()
{
	...
	printf("Введите размер массива\n");
	scanf("%d", &n);
	printf("Введите минимальное целое число\n");
	scanf("%d", &m1);
	printf("Введите максимальное целое число\n");
	scanf("%d", &m2);
	//rand15(n, mmax);
	mmax = m2 - m1+1;
	counter = (int*)calloc(mmax, sizeof(int));
	ptr_array1 = (int*)realloc(rand15(n, mmax), n * sizeof(int));
	for (int i = 0; i < n; i++)
	{
		counter[ptr_array1[i]]++;
	}
	for (int i = n; i >= 0; i--)
	{
